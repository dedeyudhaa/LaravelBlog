...
class AllPosts extends Component
{
    ...
    public function deletePost($id)
    {
        $this->dispatchBrowserEvent('deletePost',[
            ...
            'html' => 'Anda ingin menghapus post ini',
            'id' => $id
        ]);
    }
    ...
    public function deletePostAction($id)
    {
        ...
        if( $delete_post ){
            $this->showToaster('Postingan berhasil dihapus','success');
        } else{
    ...
    public function render()
    {
        return view('livewire.all-posts',[
            // KETIKA USER AUTHOR ADMIN MASUK SEMUA POSTINGAN TAMPIL, KETIKA NORMAL AUTHOR MASUK HANYA TAMPIL POSTINGAN MILIKNYA
            'posts' => auth()->user()->type == 1 ? 
                Post::search(trim($this->search))
                    ->when($this->category, function($query){
                        $query->where('subcategory_id', $this->category);
                    })
